...
list = tabula.read_pdf("Airports/EC-AD-2.1VAKE-en-GB.pdf", output_format='csv', lattice=True, pages=3, multiple_tables=True) #, area=(406, 24, 695, 589))
...
for df in list:
    clean_df = df.replace('\r',' ', regex=True) # This replaces \r in lattice mode to spaces
    export_csv = clean_df.to_csv (r'export_dataframe_clean_'+str(i)+'.csv', index = None, header=True)
    print(clean_df)
    i+=1
